...
int main(int argc, char **argv) {
    ...
    CUDACHECK(cudaStreamCreate(&stream3));
    CUDACHECK(cudaStreamCreate(&stream4));

    CUDACHECK(cudaMemcpyAsync(life_data_device, life_data_host, size, cudaMemcpyHostToDevice, stream1));
    CUDACHECK(cudaMemcpyAsync(life_data_buffer_device, life_data_buffer_host, size, cudaMemcpyHostToDevice, stream2));

    CUDAMEASURE((run_life(life_data_device, life_data_buffer_device, grid_width, grid_height, generations, tpb, stream4)));
    CUDACHECK(cudaMemcpyAsync(life_data_host, life_data_device, size, cudaMemcpyDeviceToHost, stream3));
